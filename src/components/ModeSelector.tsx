...
import { MODES, ALL_MODES, type GameMode } from '@/engine';

export default function ModeSelector() {
  const { mode, setMode, phase } = useGameStore();
  const isDisabled = phase === 'waiting' || phase === 'ready';

  return (
    <div className="w-full max-w-4xl mx-auto">
      <h2
        className="text-xl font-bold text-white/90 mb-4 text-center"
        style={{ fontFamily: "'Orbitron', sans-serif" }}
      >
        选择游戏模式
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {ALL_MODES.map((m: GameMode) => {
          const cfg = MODES[m];
          ...
          return (
            ...
            >
              ...
              <p className="text-sm text-white/60 text-center">{cfg.description}</p>
